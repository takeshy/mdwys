...
mdwys は、Markdown やドキュメントファイルをローカルで扱う軽量な desktop workspace です。ファイルを widget として開き、行・列に並べたり、Markdown を編集したり、PDF / EPUB / HTML / 画像を参照したりできます。メモは Markdown・PDF・EPUB のどれでも同じように取れて、引用箇所は本文にハイライト表示されます。
...
- Markdown / PDF / EPUB / HTML / テキストでテキストを選択して右クリック →「メモに追加」。引用と位置(アンカー)付きで投稿され、引用箇所は本文にハイライト表示。
